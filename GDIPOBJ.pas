...
\**************************************************************************)
  ...
  public
    ...
    constructor Create(rect: TRectF); reintroduce; overload;
    constructor Create(rect: TRect); reintroduce; overload;
    ...
    function Xor_(const rect: TRect): TStatus; overload;
    ...
    function GetBounds(out rect: TRect; g: TGPGraphics): TStatus; overload;
    function GetBounds(out rect: TRectF; g: TGPGraphics): TStatus; overload;
    ...
    function IsVisible(const point: TPoint; g: TGPGraphics = nil): BOOL; overload;
    function IsVisible(x, y: Single; g: TGPGraphics = nil): BOOL; overload;
    function IsVisible(const point: TPointF; g: TGPGraphics = nil): BOOL; overload;
    function IsVisible(x, y, width, height: Integer; g: TGPGraphics): BOOL; overload;
    function IsVisible(const rect: TRect; g: TGPGraphics = nil): BOOL; overload;
    function IsVisible(x, y, width, height: Single; g: TGPGraphics = nil): BOOL; overload;
    function IsVisible(const rect: TRectF; g: TGPGraphics = nil): BOOL; overload;
    function Equals(region: TGPRegion; g: TGPGraphics): BOOL;
    function GetRegionScansCount(matrix: TGPMatrix): UINT;
    function GetRegionScans(matrix: TGPMatrix ;rects: PRectF; out count: Integer): TStatus; overload;
    function GetRegionScans(matrix: TGPMatrix; rects: PRect; out count: Integer): TStatus; overload;
...
//--------------------------------------------------------------------------
  ...
  public
    ...
    function Clone(rect: TRect; format: TPixelFormat): TGPBitmap; overload;
    function Clone(x, y, width, height: Integer; format: TPixelFormat): TGPBitmap; overload;
    function Clone(rect: TRectF; format: TPixelFormat): TGPBitmap; overload;
    function Clone(x, y, width, height: Single; format: TPixelFormat): TGPBitmap; overload;
    function LockBits(rect: TRect; flags: UINT; format: TPixelFormat; out lockedBitmapData: TBitmapData): TStatus; 
    function UnlockBits(var lockedBitmapData: TBitmapData): TStatus;
    function GetPixel(x, y: Integer; out color: TColor): TStatus;
    function SetPixel(x, y: Integer; color: TColor): TStatus;
    ...
    function GetHBITMAP(colorBackground: TColor; out hbmReturn: HBITMAP): TStatus;
...
\********************************************************************F******)
  ...
  public
    ...
    function SetColorKey(colorLow, colorHigh: TColor; type_: TColorAdjustType = ColorAdjustTypeDefault): TStatus;
    ...
    function SetWrapMode(wrap: TWrapMode; color: TColor = aclBlack; clamp: BOOL = FALSE): TStatus;
...
\**************************************************************************)
  ...
  public
    ...
    constructor Create(const rect: TRectF; const dstplg: TPointF); reintroduce; overload;
    constructor Create(const rect: TRect; const dstplg: TPoint); reintroduce; overload;
    destructor Destroy; override;
    ...
    function RotateAt(angle: Single; const center: TPointF; order: TMatrixOrder = MatrixOrderPrepend): TStatus; // ok
    ...
    function TransformPoints(pts: PPoint; count: Integer = 1): TStatus; overload;

    function TransformVectors(pts: PPointF; count: Integer = 1): TStatus; overload;
    function TransformVectors(pts: PPoint; count: Integer = 1): TStatus; overload;
...
\**************************************************************************)
  ...
  public
    constructor Create(color: TColor); reintroduce; overload;
    constructor Create; reintroduce; overload;
    function GetColor(out color: TColor): TStatus;
    function SetColor(color: TColor): TStatus;
  end;
  ...
  public
    ...
    constructor Create(image: TGPImage; wrapMode: TWrapMode; dstRect: TRectF); reintroduce; overload;
    constructor Create(image: TGPImage; dstRect: TRectF; imageAttributes: TGPImageAttributes = nil); reintroduce; overload;
    constructor Create(image: TGPImage; dstRect: TRect; imageAttributes: TGPImageAttributes = nil); reintroduce; overload;
    constructor Create(image: TGPImage; wrapMode: TWrapMode; dstRect: TRect); reintroduce; overload;
  ...
  public
    ...
    constructor Create(const point1, point2: TPointF; color1,
      color2: TColor); reintroduce; overload;
    constructor Create(const point1, point2: TPoint; color1,
      color2: TColor); reintroduce; overload;
    constructor Create(rect: TRectF; color1, color2: TColor;
      mode: TLinearGradientMode); reintroduce; overload;
    constructor Create(rect: TRect; color1, color2: TColor;
      mode: TLinearGradientMode); reintroduce; overload;
    constructor Create(rect: TRectF; color1, color2: TColor; angle: Single;
      isAngleScalable: BOOL = FALSE); overload;
    constructor Create(rect: TRect; color1, color2: TColor; angle: Single;
      isAngleScalable: BOOL = FALSE); overload;
    function SetLinearColors(color1, color2: TColor): TStatus;
    function GetLinearColors(out color1, color2: TCOLOR): TStatus;
    function GetRectangle(out rect: TRectF): TStatus; overload;
    function GetRectangle(out rect: TRect): TStatus; overload;
    ...
    function SetInterpolationColors(presetColors: PColor; blendPositions: PSingle; count: Integer): TStatus;
    function GetInterpolationColors(presetColors: PColor; blendPositions: PSingle; count: Integer): TStatus;
  ...
  public
    constructor Create; reintroduce; overload;
    constructor Create(hatchStyle: THatchStyle; foreColor: TColor; backColor: TColor = aclBlack); reintroduce; overload; // ok
    function GetHatchStyle: THatchStyle;
    function GetForegroundColor(out color: TColor): TStatus;
    function GetBackgroundColor(out color: TColor): TStatus;
  end;
...
//--------------------------------------------------------------------------
  ...
  public
    constructor Create(color: TColor; width: Single = 1.0); reintroduce; overload;
...
\**************************************************************************)
  ...
  public
    ...
    constructor Create(points: PPoint; types: PBYTE; count: Integer;
      fillMode: TFillMode = FillModeAlternate); reintroduce; overload;
    destructor destroy; override;
    ...
    function AddLines(points: PPoint; count: Integer): TStatus; overload;
    ...
    function AddArc(rect: TRect; startAngle, sweepAngle: Single): TStatus; overload;
    ...
    function AddBezier(pt1, pt2, pt3, pt4: TPoint): TStatus; overload;
    function AddBezier(x1, y1, x2, y2, x3, y3, x4, y4: Integer): TStatus; overload;
    function AddBeziers(points: PPoint; count: Integer): TStatus; overload;
    ...
    function AddCurve(points: PPoint; count: Integer): TStatus; overload;
    function AddCurve(points: PPoint; count: Integer; tension: Single): TStatus; overload;
    function AddCurve(points: PPoint; count, offset, numberOfSegments: Integer; tension: Single): TStatus; overload;
    ...
    function AddClosedCurve(points: PPoint; count: Integer): TStatus; overload;
    function AddClosedCurve(points: PPoint; count: Integer; tension: Single): TStatus; overload;

    function AddRectangle(rect: TRectF): TStatus; overload;
    function AddRectangles(rects: PRectF; count: Integer): TStatus; overload;
    function AddRectangle(rect: TRect): TStatus; overload;
    function AddRectangles(rects: PRect; count: Integer): TStatus; overload;

    function AddEllipse(rect: TRectF): TStatus; overload;
    function AddEllipse(x, y, width, height: Single): TStatus; overload;
    function AddEllipse(rect: TRect): TStatus; overload;
    ...
    function AddPie(rect: TRect; startAngle, sweepAngle: Single): TStatus; overload;
    ...
    function AddPolygon(points: PPoint; count: Integer): TStatus; overload;
    ...
    function AddString(string_: WideString; length: Integer; family : TGPFontFamily;
      style  : Integer; emSize : Single; origin : TPointF; format : TGPStringFormat): TStatus; overload;
    function AddString(string_: WideString; length : Integer; family : TGPFontFamily;
      style  : Integer; emSize : Single; layoutRect: TRectF; format : TGPStringFormat): TStatus; overload;
    function AddString(string_: WideString; length : Integer; family : TGPFontFamily;
      style  : Integer; emSize : Single; origin : TPoint; format : TGPStringFormat): TStatus; overload;
    function AddString(string_: WideString; length : Integer; family : TGPFontFamily;
      style  : Integer; emSize : Single; layoutRect: TRect; format : TGPStringFormat): TStatus; overload;

    function Transform(matrix: TGPMatrix): TStatus;

    // This is not always the tightest bounds.
    function GetBounds(out bounds: TRectF; matrix: TGPMatrix = nil; pen: TGPPen = nil): TStatus; overload;
    function GetBounds(out bounds: TRect; matrix: TGPMatrix = nil; pen: TGPPen = nil): TStatus;  overload;
    ...
    function GetPathPoints(points: PPoint; count: Integer): TStatus; overload;
    function GetLastStatus: TStatus;

    function IsVisible(point: TPointF; g: TGPGraphics = nil): BOOL; overload;
    function IsVisible(x, y: Single; g: TGPGraphics = nil): BOOL; overload;
    function IsVisible(point: TPoint; g : TGPGraphics = nil): BOOL; overload;
    function IsVisible(x, y: Integer; g: TGPGraphics = nil): BOOL; overload;

    function IsOutlineVisible(point: TPointF; pen: TGPPen; g: TGPGraphics = nil): BOOL; overload;
    function IsOutlineVisible(x, y: Single; pen: TGPPen; g: TGPGraphics = nil): BOOL; overload;
    function IsOutlineVisible(point: TPoint; pen: TGPPen; g: TGPGraphics = nil): BOOL; overload;
...
//--------------------------------------------------------------------------
  ...
  public
    ...
    constructor Create(points: PPoint; count: Integer;
      wrapMode: TWrapMode = WrapModeClamp); reintroduce; overload;
    constructor Create(path: TGPGraphicsPath); reintroduce; overload;
    constructor Create; reintroduce; overload;
    function GetCenterColor(out Color: Tcolor): TStatus;
    function SetCenterColor(color: TColor): TStatus;
    function GetPointCount: Integer;
    ...
    function GetCenterPoint(out point: TPoint): TStatus; overload;
    function SetCenterPoint(point: TPointF): TStatus; overload;
    function SetCenterPoint(point: TPoint): TStatus; overload;
    function GetRectangle(out rect: TRectF): TStatus; overload;
    function GetRectangle(out rect: TRect): TStatus; overload;
...
\**************************************************************************)
  ...
  public
    ...
    function TransformPoints(destSpace: TCoordinateSpace; srcSpace: TCoordinateSpace;
      pts: PPoint; count: Integer): TStatus; overload;
    //------------------------------------------------------------------------
    // GetNearestColor (for <= 8bpp surfaces).  Note: Alpha is ignored.
    //------------------------------------------------------------------------
    function GetNearestColor(var color: TColor): TStatus;

    // DrawLine(s)
    function DrawLine(pen: TGPPen; x1, y1, x2, y2: Single): TStatus; overload;
    function DrawLine(pen: TGPPen; const pt1, pt2: TPointF): TStatus; overload;
    function DrawLines(pen: TGPPen; points: PPointF; count: Integer): TStatus; overload;
    function DrawLine(pen: TGPPen; x1, y1, x2, y2: Integer): TStatus; overload;
    function DrawLine(pen: TGPPen; const pt1, pt2: TPoint): TStatus; overload;
    function DrawLines(pen: TGPPen; points: PPoint; count: Integer): TStatus; overload;

    // DrawArc
    function DrawArc(pen: TGPPen; x, y, width, height, startAngle, sweepAngle: Single): TStatus; overload;
    function DrawArc(pen: TGPPen; const rect: TRectF; startAngle, sweepAngle: Single): TStatus; overload;
    function DrawArc(pen: TGPPen; x, y, width, height: Integer; startAngle, sweepAngle: Single): TStatus; overload;
    function DrawArc(pen: TGPPen; const rect: TRect; startAngle, sweepAngle: Single): TStatus; overload;

    // DrawBezier(s)
    function DrawBezier(pen: TGPPen; x1, y1, x2, y2, x3, y3, x4, y4: Single): TStatus; overload;
    function DrawBezier(pen: TGPPen; const pt1, pt2, pt3, pt4: TPointF): TStatus; overload;
    function DrawBeziers(pen: TGPPen; points: PPointF; count: Integer): TStatus; overload;
    function DrawBezier(pen: TGPPen; x1, y1, x2, y2, x3, y3, x4, y4: Integer): TStatus; overload;
    function DrawBezier(pen: TGPPen; const pt1, pt2, pt3, pt4: TPoint): TStatus; overload;
    function DrawBeziers(pen: TGPPen; points: PPoint; count: Integer): TStatus; overload;

    // DrawRectangle(s)
    function DrawRectangle(pen: TGPPen; const rect: TRectF): TStatus; overload;
    function DrawRectangle(pen: TGPPen; x, y, width, height: Single): TStatus; overload;
    function DrawRectangles(pen: TGPPen; rects: PRectF; count: Integer): TStatus; overload;
    function DrawRectangle(pen: TGPPen; const rect: TRect): TStatus; overload;
    function DrawRectangle(pen: TGPPen; x, y, width, height: Integer): TStatus; overload;
    function DrawRectangles(pen: TGPPen; rects: PRect; count: Integer): TStatus; overload;

    // DrawEllipse
    function DrawEllipse(pen: TGPPen; const rect: TRectF): TStatus; overload;
    function DrawEllipse(pen: TGPPen; x, y, width, height: Single): TStatus; overload;
    function DrawEllipse(pen: TGPPen; const rect: TRect): TStatus; overload;
    function DrawEllipse(pen: TGPPen; x, y, width, height: Integer): TStatus; overload;

    // DrawPie
    function DrawPie(pen: TGPPen; const rect: TRectF; startAngle, sweepAngle: Single): TStatus; overload;
    function DrawPie(pen: TGPPen; x, y, width, height, startAngle, sweepAngle: Single): TStatus; overload;
    function DrawPie(pen: TGPPen; const rect: TRect; startAngle, sweepAngle: Single): TStatus; overload;
    function DrawPie(pen: TGPPen; x, y, width, height: Integer; startAngle, sweepAngle: Single): TStatus; overload;

    // DrawPolygon
    function DrawPolygon(pen: TGPPen; points: PPointF; count: Integer): TStatus; overload;
    function DrawPolygon(pen: TGPPen; points: PPoint; count: Integer): TStatus; overload;

    // DrawPath
    function DrawPath(pen: TGPPen; path: TGPGraphicsPath): TStatus;

    // DrawCurve
    function DrawCurve(pen: TGPPen; points: PPointF; count: Integer): TStatus; overload;
    function DrawCurve(pen: TGPPen; points: PPointF; count: Integer; tension: Single): TStatus; overload;
    function DrawCurve(pen: TGPPen; points: PPointF; count, offset,
      numberOfSegments: Integer; tension: Single = 0.5): TStatus; overload;
    function DrawCurve(pen: TGPPen; points: PPoint; count: Integer): TStatus; overload;
    function DrawCurve(pen: TGPPen; points: PPoint; count: Integer; tension: Single): TStatus; overload;
    function DrawCurve(pen: TGPPen; points: PPoint; count, offset, numberOfSegments: Integer;
      tension: Single = 0.5): TStatus; overload;

    // DrawClosedCurve
    function DrawClosedCurve(pen: TGPPen; points: PPointF; count: Integer): TStatus; overload;
    function DrawClosedCurve(pen: TGPPen; points: PPointF; count: Integer; tension: Single): TStatus; overload;
    function DrawClosedCurve(pen: TGPPen; points: PPoint;  count: Integer): TStatus; overload;
    function DrawClosedCurve(pen: TGPPen; points: PPoint; count: Integer; tension: Single): TStatus; overload;

    // Clear
    function Clear(color: TColor): TStatus;

    // FillRectangle(s)
    function FillRectangle(brush: TGPBrush; const rect: TRectF): TStatus; overload;
    function FillRectangle(brush: TGPBrush; x, y, width, height: Single): TStatus; overload;
    function FillRectangles(brush: TGPBrush; rects: PRectF; count: Integer): TStatus; overload;
    function FillRectangle(brush: TGPBrush; const rect: TRect): TStatus; overload;
    function FillRectangle(brush: TGPBrush; x, y, width, height: Integer): TStatus; overload;
    function FillRectangles(brush: TGPBrush; rects: PRect; count: Integer): TStatus; overload;

    // FillPolygon
    function FillPolygon(brush: TGPBrush; points: PPointF; count: Integer): TStatus; overload;
    function FillPolygon(brush: TGPBrush; points: PPointF; count: Integer; fillMode: TFillMode): TStatus; overload;
    function FillPolygon(brush: TGPBrush; points: PPoint; count: Integer): TStatus; overload;
    function FillPolygon(brush: TGPBrush; points: PPoint; count: Integer; fillMode: TFillMode): TStatus; overload;

    // FillEllipse
    function FillEllipse(brush: TGPBrush; const rect: TRectF): TStatus; overload;
    function FillEllipse(brush: TGPBrush; x, y, width, height: Single): TStatus; overload;
    function FillEllipse(brush: TGPBrush; const rect: TRect): TStatus; overload;
    function FillEllipse(brush: TGPBrush; x, y, width, height: Integer): TStatus; overload;

    // FillPie
    function FillPie(brush: TGPBrush; const rect: TRectF; startAngle, sweepAngle: Single): TStatus; overload;
    function FillPie(brush: TGPBrush; x, y, width, height, startAngle, sweepAngle: Single): TStatus; overload;
    function FillPie(brush: TGPBrush; const rect: TRect; startAngle, sweepAngle: Single): TStatus; overload;
    ...
    // FillClosedCurve
    function FillClosedCurve(brush: TGPBrush; points: PPointF; count: Integer): TStatus; overload;
    function FillClosedCurve(brush: TGPBrush; points: PPointF; count: Integer;
      fillMode: TFillMode; tension: Single = 0.5 ): TStatus; overload;
    function FillClosedCurve(brush: TGPBrush; points: PPoint; count: Integer): TStatus; overload;
    function FillClosedCurve(brush: TGPBrush; points: PPoint; count: Integer;
      fillMode: TFillMode; tension: Single = 0.5): TStatus; overload;
    ...
    function DrawString(string_: WideString; length: Integer; font: TGPFont;
      const layoutRect: TRectF; stringFormat: TGPStringFormat; brush: TGPBrush): TStatus; overload;
    function DrawString(string_: WideString; length: Integer; font: TGPFont;
      const origin: TPointF; brush: TGPBrush): TStatus; overload;
    function DrawString(string_: WideString; length: Integer; font: TGPFont;
      const origin: TPointF; stringFormat: TGPStringFormat; brush: TGPBrush): TStatus; overload;

    // MeasureString
    function MeasureString(string_: WideString; length: Integer; font: TGPFont;
      const layoutRect: TRectF; stringFormat: TGPStringFormat; out boundingBox: TRectF;
      codepointsFitted: PInteger = nil; linesFilled: PInteger = nil): TStatus; overload;
    function MeasureString(string_: WideString; length: Integer; font: TGPFont;
      const layoutRectSize: TSizeF; stringFormat: TGPStringFormat; out size: TSizeF;
      codepointsFitted: PInteger = nil; linesFilled: PInteger = nil): TStatus; overload;
    function MeasureString(string_: WideString ; length: Integer; font: TGPFont;
      const origin: TPointF; stringFormat: TGPStringFormat;
    ...
    function MeasureCharacterRanges(string_: WideString; length: Integer; font: TGPFont;
      const layoutRect: TRectF; stringFormat: TGPStringFormat; regionCount: Integer;
      const regions: array of TGPRegion): TStatus; overload;

    // DrawDriverString
    function DrawDriverString(text: PUINT16; length: Integer; font: TGPFont;
      brush: TGPBrush; positions: PPointF; flags: Integer; matrix: TGPMatrix): TStatus;

    // MeasureDriverString
    function MeasureDriverString(text: PUINT16; length: Integer; font: TGPFont;
       positions: PPointF; flags: Integer; matrix: TGPMatrix;
    ...
    function DrawImage(image: TGPImage; const point: TPointF): TStatus; overload;
    function DrawImage(image: TGPImage; x, y: Single): TStatus; overload;
    function DrawImage(image: TGPImage; const rect: TRectF): TStatus; overload;
    function DrawImage(image: TGPImage; x, y, width, height: Single): TStatus; overload;
    function DrawImage(image: TGPImage; const point: TPoint): TStatus; overload;
    function DrawImage(image: TGPImage; x, y: Integer): TStatus; overload;
    function DrawImage(image: TGPImage; const rect: TRect): TStatus; overload;
    ...
    function DrawImage(image: TGPImage; destPoints: PPointF; count: Integer): TStatus; overload;
    function DrawImage(image: TGPImage; destPoints: PPoint; count: Integer): TStatus; overload;
    function DrawImage(image: TGPImage; x, y, srcx, srcy, srcwidth, srcheight: Single; srcUnit: TUnit): TStatus; overload;
    function DrawImage(image: TGPImage; const destRect: TRectF; srcx, srcy,
    ...
    function DrawImage(image: TGPImage; destPoints: PPointF; count: Integer;
    ...
    function DrawImage(image: TGPImage; const destRect: TRect; srcx, srcy,
      srcwidth, srcheight: Integer; srcUnit: TUnit;
      imageAttributes: TGPImageAttributes = nil; callback: DrawImageAbort = nil;
      callbackData: Pointer = nil): TStatus; overload;
    function DrawImage(image: TGPImage; destPoints: PPoint;
    ...
    function EnumerateMetafile(metafile: TGPMetafile; const destPoint: TPointF;
      callback: EnumerateMetafileProc; callbackData: Pointer = nil;
      imageAttributes: TGPImageAttributes = nil): TStatus; overload;
    function EnumerateMetafile(metafile: TGPMetafile; const destPoint: TPoint;    
       callback: EnumerateMetafileProc; callbackData: pointer = nil;
       imageAttributes: TGPImageAttributes = nil): TStatus; overload;
    function EnumerateMetafile(metafile: TGPMetafile; const destRect: TRectF;
       callback: EnumerateMetafileProc; callbackData: Pointer = nil;
       imageAttributes: TGPImageAttributes = nil): TStatus; overload;
    function EnumerateMetafile(metafile: TGPMetafile; const destRect: TRect;
       callback: EnumerateMetafileProc; callbackData: Pointer = nil;
       imageAttributes: TGPImageAttributes = nil): TStatus; overload;
    function EnumerateMetafile(metafile: TGPMetafile; destPoints: PPointF;
       count: Integer; callback: EnumerateMetafileProc; callbackData: Pointer = nil;
       imageAttributes: TGPImageAttributes = nil): TStatus; overload;
    function EnumerateMetafile(metafile: TGPMetafile; destPoints: PPoint;
       count: Integer; callback: EnumerateMetafileProc; callbackData: Pointer = nil;
       imageAttributes: TGPImageAttributes = nil): TStatus; overload;
    function EnumerateMetafile(metafile: TGPMetafile; const destPoint: TPointF;
    ...
    function EnumerateMetafile(metafile : TGPMetafile; const destPoint : TPoint;
       const srcRect : TRect; srcUnit : TUnit; callback : EnumerateMetafileProc;
       callbackData : Pointer = nil; imageAttributes : TGPImageAttributes = nil
       ): TStatus; overload;
    function EnumerateMetafile(metafile: TGPMetafile; const destRect: TRectF;
       const srcRect: TRectF; srcUnit: TUnit; callback: EnumerateMetafileProc;
       callbackData: Pointer = nil; imageAttributes: TGPImageAttributes = nil): TStatus; overload;
    function EnumerateMetafile(metafile : TGPMetafile; const destRect, srcRect: TRect;
    ...
    function EnumerateMetafile( metafile: TGPMetafile; destPoints: PPointF;
        ...
        callbackData: Pointer = nil; imageAttributes: TGPImageAttributes = nil): TStatus; overload;
    function EnumerateMetafile(metafile: TGPMetafile; destPoints: PPoint;
        count: Integer; const srcRect: TRect; srcUnit: TUnit; callback: EnumerateMetafileProc;
        callbackData: Pointer = nil; imageAttributes: TGPImageAttributes = nil): TStatus; overload;
    ...
    function SetClip(rect: TRect; combineMode: TCombineMode = CombineModeReplace): TStatus; overload;
    ...
    function GetClipBounds(out rect: TRect): TStatus; overload;

    function IsClipEmpty: Bool;

    function GetVisibleClipBounds(out rect: TRectF): TStatus; overload;
    function GetVisibleClipBounds(out rect: TRect): TStatus; overload;
    ...
    function BeginContainer(const dstrect, srcrect: TRect; unit_: TUnit): GraphicsContainer; overload;
...
\**************************************************************************)
  ...
  public
    ...
    // Record a metafile to memory.
    constructor Create(referenceHdc: HDC; frameRect: TRectF;
      frameUnit: TMetafileFrameUnit = MetafileFrameUnitGdi;
      type_: TEmfType = EmfTypeEmfPlusDual; description: PWCHAR = nil); overload;
    // Record a metafile to memory.
    constructor Create(referenceHdc: HDC; frameRect: TRect;
      frameUnit: TMetafileFrameUnit = MetafileFrameUnitGdi;
      type_: TEmfType = EmfTypeEmfPlusDual; description: PWCHAR = nil); overload;
    constructor Create(fileName: WideString;referenceHdc: HDC;
      type_: TEmfType = EmfTypeEmfPlusDual; description: PWCHAR = nil); overload;
    constructor Create(fileName: WideString; referenceHdc: HDC; frameRect: TRectF;
      frameUnit: TMetafileFrameUnit = MetafileFrameUnitGdi;
      type_: TEmfType = EmfTypeEmfPlusDual; description: PWCHAR = nil); overload;
    constructor Create( fileName: WideString; referenceHdc: HDC; frameRect: TRect;
      frameUnit: TMetafileFrameUnit = MetafileFrameUnitGdi;
      type_: TEmfType = EmfTypeEmfPlusDual; description: PWCHAR = nil); overload;
    constructor Create(stream: IStream; referenceHdc: HDC;
      type_: TEmfType = EmfTypeEmfPlusDual; description: PWCHAR = nil); overload;
    constructor Create(stream: IStream; referenceHdc: HDC; frameRect: TRectF;
      frameUnit: TMetafileFrameUnit = MetafileFrameUnitGdi;
      type_: TEmfType = EmfTypeEmfPlusDual; description: PWCHAR = nil); overload;
    constructor Create(stream : IStream; referenceHdc : HDC; frameRect : TRect;
     frameUnit : TMetafileFrameUnit = MetafileFrameUnitGdi;
     type_ : TEmfType = EmfTypeEmfPlusDual; description : PWCHAR = nil); overload;
    constructor Create; reintroduce; overload;
...
\********************************************************************F******)
  ...
  function TGPImageAttributes.SetColorKey(colorLow, colorHigh: TColor;
    type_: TColorAdjustType = ColorAdjustTypeDefault): TStatus;
  begin
  ...
  function TGPImageAttributes.SetWrapMode(wrap: TWrapMode; color: TColor = aclBlack;
...
\**************************************************************************)
  ...
  constructor TGPMatrix.Create(const rect: TRectF; const dstplg: TPointF);
  var matrix: GpMatrix;
  begin
    matrix := nil;
  ...
  constructor TGPMatrix.Create(const rect: TRect; const dstplg: TPoint);
  var matrix: GpMatrix;
  begin
    matrix := nil;
  ...
  function TGPMatrix.RotateAt(angle: Single; const center: TPointF; order: TMatrixOrder = MatrixOrderPrepend): TStatus;
  begin
  ...
  // float version
  function TGPMatrix.TransformPoints(pts: PPointF; count: Integer = 1): TStatus;
  begin
    result := SetStatus(GdipTransformMatrixPoints(nativeMatrix, pts, count));
  end;

  function TGPMatrix.TransformPoints(pts: PPoint; count: Integer = 1): TStatus;
  begin
    result := SetStatus(GdipTransformMatrixPointsI(nativeMatrix, pts, count));
  end;

  function TGPMatrix.TransformVectors(pts: PPointF; count: Integer = 1): TStatus;
  begin
    result := SetStatus(GdipVectorTransformMatrixPoints( nativeMatrix, pts, count));
  end;

  function TGPMatrix.TransformVectors(pts: PPoint; count: Integer = 1): TStatus;
  begin
...
\**************************************************************************)
  ...
  constructor TGPMetafile.Create(referenceHdc: HDC; frameRect: TRectF;
     frameUnit: TMetafileFrameUnit = MetafileFrameUnitGdi;
  ...
  constructor TGPMetafile.Create(referenceHdc: HDC; frameRect: TRect;
    frameUnit: TMetafileFrameUnit = MetafileFrameUnitGdi;
    type_: TEmfType = EmfTypeEmfPlusDual; description: PWCHAR = nil);
  var
    metafile: GpMetafile;
  ...
  constructor TGPMetafile.Create(fileName: WideString; referenceHdc: HDC; frameRect: TRectF;
  ...
  constructor TGPMetafile.Create(fileName: WideString; referenceHdc: HDC; frameRect: TRect;
    frameUnit: TMetafileFrameUnit = MetafileFrameUnitGdi; type_: TEmfType = EmfTypeEmfPlusDual;
    description: PWCHAR = nil);
  var
    metafile: GpMetafile;
  ...
  constructor TGPMetafile.Create(stream: IStream; referenceHdc: HDC; frameRect: TRectF;
    frameUnit: TMetafileFrameUnit = MetafileFrameUnitGdi; type_: TEmfType = EmfTypeEmfPlusDual;
    description: PWCHAR = nil);
  var
    metafile: GpMetafile;
  ...
  constructor TGPMetafile.Create(stream : IStream; referenceHdc : HDC; frameRect : TRect;
    frameUnit : TMetafileFrameUnit = MetafileFrameUnitGdi; type_ : TEmfType = EmfTypeEmfPlusDual;
    description : PWCHAR = nil);
  var
    metafile: GpMetafile;
...
\**************************************************************************)
  ...
  constructor TGPRegion.Create(rect: TRectF);
  var
    region: GpRegion;
  begin
    region := nil;
  ...
  constructor TGPRegion.Create(rect: TRect);
  var
    region: GpRegion;
  begin
    region := nil;
  ...
  function TGPRegion.Intersect(const rect: TRect): TStatus;
  begin
    result := SetStatus(GdipCombineRegionRectI(nativeRegion, @rect, CombineModeIntersect));
  end;

  function TGPRegion.Intersect(const rect: TRectF): TStatus;
  begin
  ...
  function TGPRegion.Union(const rect: TRect): TStatus;
  begin
    result := SetStatus(GdipCombineRegionRectI(nativeRegion, @rect, CombineModeUnion));
  end;

  function TGPRegion.Union(const rect: TRectF): TStatus;
  begin
  ...
  function TGPRegion.Xor_(const rect: TRect): TStatus;
  begin
    result := SetStatus(GdipCombineRegionRectI(nativeRegion, @rect, CombineModeXor));
  end;

  function TGPRegion.Xor_(const rect: TRectF): TStatus;
  begin
  ...
  function TGPRegion.Exclude(const rect: TRect): TStatus;
  begin
     result := SetStatus(GdipCombineRegionRectI(nativeRegion, @rect, CombineModeExclude));
  end;

  function TGPRegion.Exclude(const rect: TRectF): TStatus;
  begin
  ...
  function TGPRegion.Complement(const rect: TRect): TStatus;
  begin
    result := SetStatus(GdipCombineRegionRectI(nativeRegion, @rect,
                                                        CombineModeComplement));
  end;

  function TGPRegion.Complement(const rect: TRectF): TStatus;
  begin
  ...
  function TGPRegion.GetBounds(out rect: TRect; g: TGPGraphics): TStatus;
  begin
  ...
  function TGPRegion.GetBounds(out rect: TRectF; g: TGPGraphics): TStatus;
  begin
  ...
  function TGPRegion.IsVisible(const point: TPoint; g: TGPGraphics = nil): BOOL;
  var
    booln: BOOL;
    GPX: GpGraphics;
  begin
  ...
  function TGPRegion.IsVisible(const point: TPointF; g: TGPGraphics = nil): BOOL;
  var
    booln: BOOL;
    GPX: GpGraphics;
  begin
  ...
  function TGPRegion.IsVisible(const rect: TRect; g: TGPGraphics = nil): BOOL;
  var
    booln: BOOL;
    GPX: GpGraphics;
  begin
  ...
  function TGPRegion.IsVisible(const rect: TRectF; g: TGPGraphics = nil): BOOL;
  var
    booln: BOOL;
    GPX: GpGraphics;
  begin
  ...
  function TGPRegion.GetRegionScans(matrix: TGPMatrix; rects: PRectF; out count: Integer): TStatus;
  begin
  ...
  function TGPRegion.GetRegionScans(matrix: TGPMatrix; rects: PRect; out count: Integer): TStatus;
  begin
...
\**************************************************************************)
  ...
  constructor TGPPen.Create(color: TColor; width: Single = 1.0);
  var unit_: TUnit;
  begin
    unit_ := UnitWorld;
    nativePen := nil;
  ...
  function TGPPen.SetColor(color: TColor): TStatus;
  begin
  ...
  function TGPPen.GetColor(out Color: TColor): TStatus;
  var
    type_: TPenType;
    argb: DWORD;
  begin
...
//--------------------------------------------------------------------------
  ...
  constructor TGPSolidBrush.Create(color: TColor);
  var
    brush: GpSolidFill;
  begin
    brush := nil;
  ...
  function TGPSolidBrush.GetColor(out color: TColor): TStatus;
  begin
    SetStatus(GdipGetSolidFillColor(GPSOLIDFILL(nativeBrush), color));
    result := lastResult;
  end;

  function TGPSolidBrush.SetColor(color: TColor): TStatus;
  begin
...
//--------------------------------------------------------------------------
  ...
  constructor TGPTextureBrush.Create(image: TGPImage; wrapMode: TWrapMode; dstRect: TRectF);
  var texture: GpTexture;
  begin
    texture := nil;
  ...
  constructor TGPTextureBrush.Create(image: TGPImage; dstRect: TRectF; imageAttributes: TGPImageAttributes = nil);
  var texture: GpTexture;
  ...
  constructor TGPTextureBrush.Create(image: TGPImage; dstRect: TRect; imageAttributes: TGPImageAttributes = nil);
  var texture: GpTexture;
      ImgAtt: GpImageAttributes;
  begin
    texture := nil;
  ...
  constructor TGPTextureBrush.Create(image: TGPImage; wrapMode: TWrapMode; dstRect: TRect);
  var texture: GpTexture;
  begin
    texture := nil;
...
//--------------------------------------------------------------------------
  ...
  constructor TGPLinearGradientBrush.Create(const point1, point2: TPointF; color1, color2: TColor);
  ...
  constructor TGPLinearGradientBrush.Create(const point1, point2: TPoint; color1, color2: TColor);
  var brush: GpLineGradient;
  begin
    brush := nil;
  ...
  constructor TGPLinearGradientBrush.Create(rect: TRectF; color1, color2: TColor; mode: TLinearGradientMode);
  var brush: GpLineGradient;
  begin
    brush := nil;
  ...
  constructor TGPLinearGradientBrush.Create(rect: TRect; color1, color2: TColor; mode: TLinearGradientMode);
  var brush: GpLineGradient;
  begin
    brush := nil;
  ...
  constructor TGPLinearGradientBrush.Create(rect: TRectF; color1, color2: TColor; angle: Single; isAngleScalable: BOOL = FALSE);
  var brush: GpLineGradient;
  begin
    brush := nil;
  ...
  constructor TGPLinearGradientBrush.Create(rect: TRect; color1, color2: TColor; angle: Single; isAngleScalable: BOOL = FALSE);
  var brush: GpLineGradient;
  begin
    brush := nil;
  ...
  function TGPLinearGradientBrush.SetLinearColors(color1, color2: TColor): TStatus;
  begin
    result := SetStatus(GdipSetLineColors(GpLineGradient(nativeBrush),
                color1, color2));
  end;

  function TGPLinearGradientBrush.GetLinearColors(out color1, color2: TCOLOR): TStatus;
  var colors: array[0..1] of TColor;
  begin
  ...
  function TGPLinearGradientBrush.GetRectangle(out rect: TRectF): TStatus;
  begin
    result := SetStatus(GdipGetLineRect(GpLineGradient(nativeBrush), @rect));
  end;

  function TGPLinearGradientBrush.GetRectangle(out rect: TRect): TStatus;
  begin
  ...
  function TGPLinearGradientBrush.SetInterpolationColors(presetColors: PColor;
    blendPositions: PSingle; count: Integer): TStatus;
  begin
    if (count <= 0)  then
      result := SetStatus(InvalidParameter)
  ...
  function TGPLinearGradientBrush.GetInterpolationColors(presetColors: PColor; blendPositions: PSingle; count: Integer): TStatus;
  begin
    if (count <= 0) then
      result := SetStatus(InvalidParameter)
    else
...
//--------------------------------------------------------------------------
  ...
  constructor TGPHatchBrush.Create(hatchStyle: THatchStyle; foreColor: TColor; backColor: TColor = aclBlack);
  var
    brush: GpHatch;
  begin
    brush := nil;
  ...
  function TGPHatchBrush.GetForegroundColor(out color: TColor): TStatus;
  begin
    result := SetStatus(GdipGetHatchForegroundColor(GpHatch(nativeBrush), color));
  end;

  function TGPHatchBrush.GetBackgroundColor(out color: TColor): TStatus;
  begin
  ...
  function TGPImage.GetPhysicalDimension(out size: TSizeF): TStatus;
  var
    width, height: Single;
    status: TStatus;
  begin
  ...
  function TGPImage.GetBounds(out srcRect: TRectF; out srcUnit: TUnit): TStatus;
  begin
  ...
  function TGPBitmap.Clone(rect: TRect; format: TPixelFormat): TGPBitmap;
  begin
  ...
  function TGPBitmap.Clone(rect: TRectF; format: TPixelFormat): TGPBitmap;
  begin
    result := Clone(rect.X, rect.Y, rect.Width, rect.Height, format);
  end;
  ...
  function TGPBitmap.LockBits(rect: TRect; flags: UINT; format: TPixelFormat;
  ...
  function TGPBitmap.GetPixel(x, y: Integer; out color: TColor): TStatus;
  begin
    result := SetStatus(GdipBitmapGetPixel(GpBitmap(nativeImage), x, y, color));
  end;

  function TGPBitmap.SetPixel(x, y: Integer; color: TColor): TStatus;
  begin
  ...
  function TGPBitmap.GetHBITMAP(colorBackground: TColor; out hbmreturn: HBITMAP): TStatus;
  begin
...
\**************************************************************************)
  ...
  function TGPGraphics.GetNearestColor(var color: TColor): TStatus;
  begin
  ...
  function TGPGraphics.DrawLine(pen: TGPPen; const pt1, pt2: TPointF): TStatus;
  begin
    result := DrawLine(pen, pt1.X, pt1.Y, pt2.X, pt2.Y);
  end;

  function TGPGraphics.DrawLines(pen: TGPPen; points: PPointF; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.DrawLine(pen: TGPPen; const pt1, pt2: TPoint): TStatus;
  begin
    result := DrawLine(pen,
            pt1.X,
            pt1.Y,
  ...
  function TGPGraphics.DrawLines(pen: TGPPen; points: PPoint; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.DrawArc(pen: TGPPen; const rect: TRectF; startAngle, sweepAngle: Single): TStatus;
  begin
  ...
  function TGPGraphics.DrawArc(pen: TGPPen; const rect: TRect; startAngle, sweepAngle: Single): TStatus;
  begin
    result := DrawArc(pen,
               rect.X,
               rect.Y,
  ...
  function TGPGraphics.DrawBezier(pen: TGPPen; const pt1, pt2, pt3, pt4: TPointF): TStatus;
  begin
  ...
  function TGPGraphics.DrawBeziers(pen: TGPPen; points: PPointF; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.DrawBezier(pen: TGPPen; const pt1, pt2, pt3, pt4: TPoint): TStatus;
  begin
    result := DrawBezier(pen,
              pt1.X,
              pt1.Y,
  ...
  function TGPGraphics.DrawBeziers(pen: TGPPen; points: PPoint; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.DrawRectangle(pen: TGPPen; const rect: TRectF): TStatus;
  begin
  ...
  function TGPGraphics.DrawRectangles(pen: TGPPen;  rects: PRectF; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.DrawRectangle(pen: TGPPen; const rect: TRect): TStatus;
  begin
  ...
  function TGPGraphics.DrawRectangles(pen: TGPPen; rects: PRect; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.DrawEllipse(pen: TGPPen; const rect: TRectF): TStatus;
  begin
  ...
  function TGPGraphics.DrawEllipse(pen: TGPPen; const rect: TRect): TStatus;
  begin
  ...
  function TGPGraphics.DrawPie(pen: TGPPen; const rect: TRectF; startAngle, sweepAngle: Single): TStatus;
  begin
    result := DrawPie(pen,
               rect.X,
               rect.Y,
  ...
  function TGPGraphics.DrawPie(pen: TGPPen; const rect: TRect; startAngle, sweepAngle: Single): TStatus;
  begin
    result := DrawPie(pen,
               rect.X,
               rect.Y,
  ...
  function TGPGraphics.DrawPolygon(pen: TGPPen; points: PPointF; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.DrawPolygon(pen: TGPPen; points: PPoint; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.DrawCurve(pen: TGPPen; points: PPointF; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.DrawCurve(pen: TGPPen; points: PPointF; count: Integer; tension: Single): TStatus;
  begin
  ...
  function TGPGraphics.DrawCurve(pen: TGPPen; points: PPointF; count, offset,
  ...
  function TGPGraphics.DrawCurve(pen: TGPPen; points: PPoint; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.DrawCurve(pen: TGPPen; points: PPoint; count: Integer; tension: Single): TStatus;
  begin
  ...
  function TGPGraphics.DrawCurve(pen: TGPPen; points: PPoint; count, offset,
           numberOfSegments: Integer; tension: Single = 0.5): TStatus; 
  begin
  ...
  function TGPGraphics.DrawClosedCurve(pen: TGPPen; points: PPointF; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.DrawClosedCurve(pen: TGPPen; points: PPointF; count: Integer;
  ...
  function TGPGraphics.DrawClosedCurve(pen: TGPPen; points: PPoint; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.DrawClosedCurve(pen: TGPPen; points: PPoint;
  ...
  function TGPGraphics.Clear(color: TColor): TStatus;
  begin
  ...
  function TGPGraphics.FillRectangle(brush: TGPBrush; const rect: TRectF): TStatus;
  begin
  ...
  function TGPGraphics.FillRectangles(brush: TGPBrush; rects: PRectF; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.FillRectangle(brush: TGPBrush; const rect: TRect): TStatus;
  begin
  ...
  function TGPGraphics.FillRectangles(brush: TGPBrush; rects: PRect; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.FillPolygon(brush: TGPBrush; points: PPointF; count: Integer): TStatus;
  begin
    result := FillPolygon(brush, points, count, FillModeAlternate);
  end;

  function TGPGraphics.FillPolygon(brush: TGPBrush; points: PPointF; count: Integer;
  ...
  function TGPGraphics.FillPolygon(brush: TGPBrush; points: PPoint; count: Integer): TStatus;
  begin
    result := FillPolygon(brush, points, count, FillModeAlternate);
  end;

  function TGPGraphics.FillPolygon(brush: TGPBrush; points: PPoint; count: Integer;
               fillMode: TFillMode): TStatus;
  begin
  ...
  function TGPGraphics.FillEllipse(brush: TGPBrush; const rect: TRectF): TStatus;
  begin
  ...
  function TGPGraphics.FillEllipse(brush: TGPBrush; const rect: TRect): TStatus;
  begin
    result := FillEllipse(brush, rect.X, rect.Y, rect.Width, rect.Height);
  end;
  ...
  function TGPGraphics.FillPie(brush: TGPBrush; const rect: TRectF; startAngle, sweepAngle: Single): TStatus;
  begin
  ...
  function TGPGraphics.FillPie(brush: TGPBrush; const rect: TRect; startAngle, sweepAngle: Single): TStatus;
  begin
    result := FillPie(brush, rect.X, rect.Y, rect.Width, rect.Height,
               startAngle, sweepAngle);
  end;
  ...
  function TGPGraphics.FillClosedCurve(brush: TGPBrush; points: PPointF; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.FillClosedCurve(brush: TGPBrush; points: PPointF; count: Integer;
  ...
  function TGPGraphics.FillClosedCurve(brush: TGPBrush; points:  PPoint; count: Integer): TStatus;
  begin
  ...
  function TGPGraphics.FillClosedCurve(brush: TGPBrush; points: PPoint;
  ...
  function TGPGraphics.DrawString( string_: WideString; length: Integer; font: TGPFont;
     const layoutRect: TRectF; stringFormat: TGPStringFormat; brush: TGPBrush): TStatus;
  var
    nFont: GpFont;
  ...
  function TGPGraphics.DrawString(string_: WideString; length: Integer; font: TGPFont;
           const origin: TPointF; brush: TGPBrush): TStatus;
  var
    rect: TRectF;
    nfont: Gpfont;
    nBrush: GpBrush;
  ...
  function TGPGraphics.DrawString(string_: WideString; length: Integer; font: TGPFont;
      const origin: TPointF; stringFormat: TGPStringFormat; brush: TGPBrush): TStatus;
  var
    rect: TRectF;
    nFont: GpFont;
    nStringFormat: GpStringFormat;
  ...
  function TGPGraphics.MeasureString(string_: WideString ; length: Integer; font: TGPFont;
       const origin: TPointF; stringFormat: TGPStringFormat; out boundingBox: TRectF): TStatus;
  var
    rect: TRectF;
    nFont: GpFont;
    nstringFormat: GpstringFormat;
  ...
  function TGPGraphics.DrawDriverString(text: PUINT16; length: Integer; font: TGPFont
       ; brush: TGPBrush; positions: PPointF; flags: Integer
  ...
  function TGPGraphics.DrawImage(image: TGPImage; const point: TPointF): TStatus;
  begin
  ...
  function TGPGraphics.DrawImage(image: TGPImage; const rect: TRectF): TStatus;
  begin
  ...
  function TGPGraphics.DrawImage(image: TGPImage; const point: TPoint): TStatus;
  begin
    result := DrawImage(image, point.X, point.Y);
  end;
  ...
  function TGPGraphics.DrawImage(image: TGPImage; const rect: TRect): TStatus;
  begin
  ...
  function TGPGraphics.DrawImage(image: TGPImage; destPoints: PPointF; count: Integer): TStatus;
  var
   nImage: GpImage;
  begin
  ...
  function TGPGraphics.DrawImage(image: TGPImage; destPoints: PPoint; count: Integer): TStatus;
  var
   nImage: GpImage;
  begin
    if (((count <> 3) and (count <> 4))or (destPoints = nil)) then
  ...
  function TGPGraphics.DrawImage(image: TGPImage; const destRect: TRectF; srcx, srcy, srcwidth, srcheight: Single;
  ...
  function TGPGraphics.DrawImage(image: TGPImage; destPoints: PPointF; count: Integer;
       srcx, srcy, srcwidth, srcheight: Single; srcUnit: TUnit;
       imageAttributes: TGPImageAttributes = nil; callback: DrawImageAbort = nil;
       callbackData: Pointer = nil): TStatus;
  var
  ...
  function TGPGraphics.DrawImage(image: TGPImage; const destRect: TRect; srcx, srcy, srcwidth,
  ...
  function TGPGraphics.DrawImage(image: TGPImage; destPoints: PPoint;
       count, srcx, srcy, srcwidth, srcheight: Integer; srcUnit: TUnit;
       imageAttributes: TGPImageAttributes = nil; callback: DrawImageAbort = nil;
       callbackData: Pointer = nil): TStatus;
  var
  ...
  function TGPGraphics.EnumerateMetafile(metafile: TGPMetafile; const destPoint: TPointF;
      callback: EnumerateMetafileProc; callbackData: Pointer = nil;
  ...
  function TGPGraphics.EnumerateMetafile(metafile: TGPMetafile; const destPoint: TPoint;
       callback: EnumerateMetafileProc; callbackData: pointer = nil;
       imageAttributes: TGPImageAttributes = nil): TStatus;
  var
    nMetafile: GpMetafile;
  ...
  function TGPGraphics.EnumerateMetafile(metafile: TGPMetafile; const destRect: TRectF;
       callback: EnumerateMetafileProc; callbackData: Pointer = nil;
       imageAttributes: TGPImageAttributes = nil): TStatus;
  var
    nMetafile: GpMetafile;
  ...
  function TGPGraphics.EnumerateMetafile(metafile: TGPMetafile; const destRect: TRect;
       callback: EnumerateMetafileProc; callbackData: Pointer = nil;
       imageAttributes: TGPImageAttributes = nil): TStatus;
  var
    nMetafile: GpMetafile;
  ...
  function TGPGraphics.EnumerateMetafile(metafile: TGPMetafile; destPoints: PPointF;
       count: Integer; callback: EnumerateMetafileProc; callbackData: Pointer = nil;
       imageAttributes: TGPImageAttributes = nil): TStatus;
  var
    nMetafile: GpMetafile;
  ...
  function TGPGraphics.EnumerateMetafile(metafile: TGPMetafile; destPoints: PPoint;
       count: Integer; callback: EnumerateMetafileProc; callbackData: Pointer = nil;
       imageAttributes: TGPImageAttributes = nil): TStatus;
  var
    nMetafile: GpMetafile;
  ...
  function TGPGraphics.EnumerateMetafile(metafile : TGPMetafile; const destRect, srcRect: TRect;
       srcUnit : TUnit; callback : EnumerateMetafileProc; callbackData : Pointer = nil;
       imageAttributes : TGPImageAttributes = nil): TStatus;
  var
    nMetafile: GpMetafile;
  ...
  function TGPGraphics.SetClip(rect: TRectF; combineMode: TCombineMode = CombineModeReplace): TStatus;
  begin
  ...
  function TGPGraphics.SetClip(rect: TRect; combineMode: TCombineMode = CombineModeReplace): TStatus;
  begin
  ...
  function TGPGraphics.IntersectClip(const rect: TRectF): TStatus;
  begin
    result := SetStatus(GdipSetClipRect(nativeGraphics,
                             rect.X, rect.Y,
                             rect.Width, rect.Height,
  ...
  function TGPGraphics.IntersectClip(const rect: TRect): TStatus;
  begin
    result := SetStatus(GdipSetClipRectI(nativeGraphics,
                              rect.X, rect.Y,
                              rect.Width, rect.Height,
  ...
  function TGPGraphics.ExcludeClip(const rect: TRectF): TStatus;
  begin
    result := SetStatus(GdipSetClipRect(nativeGraphics,
                             rect.X, rect.Y,
                             rect.Width, rect.Height,
  ...
  function TGPGraphics.ExcludeClip(const rect: TRect): TStatus;
  begin
    result := SetStatus(GdipSetClipRectI(nativeGraphics,
                              rect.X, rect.Y,
                              rect.Width, rect.Height,
  ...
  function TGPGraphics.GetClipBounds(out rect: TRectF): TStatus;
  begin
    result := SetStatus(GdipGetClipBounds(nativeGraphics, @rect));
  end;

  function TGPGraphics.GetClipBounds(out rect: TRect): TStatus;
  begin
  ...
  function TGPGraphics.GetVisibleClipBounds(out rect: TRectF): TStatus;
  begin
    result := SetStatus(GdipGetVisibleClipBounds(nativeGraphics, @rect));
  end;

  function TGPGraphics.GetVisibleClipBounds(out rect: TRect): TStatus;
  begin
  ...
  function TGPGraphics.IsVisible(const point: TPoint): BOOL;
  var booln: BOOL;
  begin
    booln := FALSE;
  ...
  function TGPGraphics.IsVisible(const rect: TRect): BOOL;
  var booln: BOOL;
  begin
    booln := TRUE;
    SetStatus(GdipIsVisibleRectI(nativeGraphics,
  ...
  function TGPGraphics.IsVisible(const point: TPointF): BOOL;
  var booln: BOOL;
  begin
    booln := FALSE;
    SetStatus(GdipIsVisiblePoint(nativeGraphics,
  ...
  function TGPGraphics.IsVisible(const rect: TRectF): BOOL;
  var booln: BOOL;
  begin
    booln := TRUE;
    SetStatus(GdipIsVisibleRect(nativeGraphics,
  ...
  function TGPGraphics.BeginContainer(const dstrect, srcrect: TRect; unit_: TUnit): GraphicsContainer;
  begin
...
\**************************************************************************)
  ...
  constructor TGPGraphicsPath.Create(points: PPointF; types: PBYTE; count: Integer;
  ...
  constructor TGPGraphicsPath.Create(points: PPoint; types: PBYTE; count: Integer;
              fillMode: TFillMode = FillModeAlternate);
  begin
      nativePath := nil;
  ...
  function TGPGraphicsPath.GetLastPoint(out lastPoint: TPointF): TStatus;
  begin
    result := SetStatus(GdipGetPathLastPoint(nativePath,
                                            @lastPoint));
  end;

  function TGPGraphicsPath.AddLine(const pt1, pt2: TPointF): TStatus;
  begin
  ...
  function TGPGraphicsPath.AddLines(points: PPointF; count: Integer): TStatus;
  begin
    result := SetStatus(GdipAddPathLine2(nativePath, points, count));
  end;

  function TGPGraphicsPath.AddLine(const pt1, pt2: TPoint): TStatus;
  begin
    result := AddLine(pt1.X, pt1.Y, pt2.X, pt2.Y);
  end;
  ...
  function TGPGraphicsPath.AddLines(points: PPoint; count: Integer): TStatus;
  begin
    result := SetStatus(GdipAddPathLine2I(nativePath, points, count));
  end;

  function TGPGraphicsPath.AddArc(rect: TRectF; startAngle, sweepAngle: Single): TStatus;
  begin
  ...
  function TGPGraphicsPath.AddArc(rect: TRect; startAngle, sweepAngle: Single): TStatus;
  begin
  ...
  function TGPGraphicsPath.AddBezier(pt1, pt2, pt3, pt4: TPointF): TStatus;
  begin
  ...
  function TGPGraphicsPath.AddBeziers(points: PPointF; count: Integer): TStatus;
  begin
    result := SetStatus(GdipAddPathBeziers(nativePath, points, count));
  end;

  function TGPGraphicsPath.AddBezier(pt1, pt2, pt3, pt4: TPoint): TStatus;
  begin
    result := AddBezier(pt1.X, pt1.Y, pt2.X, pt2.Y, pt3.X, pt3.Y, pt4.X, pt4.Y);
  end;
  ...
  function TGPGraphicsPath.AddBeziers(points: PPoint; count: Integer): TStatus;
  begin
       result := SetStatus(GdipAddPathBeziersI(nativePath, points, count));
  end;

  function TGPGraphicsPath.AddCurve(points: PPointF; count: Integer): TStatus;
  begin
    result := SetStatus(GdipAddPathCurve(nativePath, points, count));
  end;

  function TGPGraphicsPath.AddCurve(points: PPointF; count: Integer;
    tension: Single): TStatus;
  begin
    result := SetStatus(GdipAddPathCurve2(nativePath, points, count, tension));
  end;

  function TGPGraphicsPath.AddCurve(points: PPointF; count, offset,
  ...
  function TGPGraphicsPath.AddCurve(points: PPoint; count: Integer): TStatus;
  begin
    result := SetStatus(GdipAddPathCurveI(nativePath, points, count));
  end;

  function TGPGraphicsPath.AddCurve(points: PPoint; count: Integer; tension: Single): TStatus;
  begin
    result := SetStatus(GdipAddPathCurve2I(nativePath, points, count, tension));
  end;

  function TGPGraphicsPath.AddCurve(points: PPoint; count, offset,
    numberOfSegments: Integer; tension: Single): TStatus;
  begin
  ...
  function TGPGraphicsPath.AddClosedCurve(points: PPointF; count: Integer): TStatus;
  begin
    result := SetStatus(GdipAddPathClosedCurve(nativePath, points, count));
  end;

  function TGPGraphicsPath.AddClosedCurve(points: PPointF; count: Integer; tension: Single): TStatus;
  begin
    result := SetStatus(GdipAddPathClosedCurve2(nativePath, points, count, tension));
  end;

  function TGPGraphicsPath.AddClosedCurve(points: PPoint; count: Integer): TStatus;
  begin
    result := SetStatus(GdipAddPathClosedCurveI(nativePath, points, count));
  end;


  function TGPGraphicsPath.AddClosedCurve(points: PPoint; count: Integer; tension: Single): TStatus;
  begin
       result := SetStatus(GdipAddPathClosedCurve2I(nativePath, points, count, tension));
  end;

  function TGPGraphicsPath.AddRectangle(rect: TRectF): TStatus;
  begin
  ...
  function TGPGraphicsPath.AddRectangles(rects: PRectF; count: Integer): TStatus;
  begin
  ...
  function TGPGraphicsPath.AddRectangle(rect: TRect): TStatus;
  begin
  ...
  function TGPGraphicsPath.AddRectangles(rects: PRect; count: Integer): TStatus;
  begin
  ...
  function TGPGraphicsPath.AddEllipse(rect: TRectF): TStatus;
  begin
  ...
  function TGPGraphicsPath.AddEllipse(rect: TRect): TStatus;
  begin
    result := AddEllipse(rect.X, rect.Y, rect.Width, rect.Height);
  end;
  ...
  function TGPGraphicsPath.AddPie(rect: TRectF; startAngle, sweepAngle: Single): TStatus;
  begin
  ...
  function TGPGraphicsPath.AddPie(rect: TRect; startAngle, sweepAngle: Single): TStatus;
  begin
  ...
  function TGPGraphicsPath.AddPolygon(points: PPointF; count: Integer): TStatus;
  begin
    result := SetStatus(GdipAddPathPolygon(nativePath, points, count));
  end;

  function TGPGraphicsPath.AddPolygon(points: PPoint; count: Integer): TStatus;
  begin
  ...
  function TGPGraphicsPath.GetBounds(out bounds: TRectF; matrix: TGPMatrix = nil; pen: TGPPen = nil): TStatus;
  var
    nativeMatrix: GpMatrix;
    nativePen: GpPen;
  begin
  ...
  function TGPGraphicsPath.GetBounds(out bounds: TRect; matrix: TGPMatrix = nil; pen: TGPPen = nil): TStatus;
  var
    nativeMatrix: GpMatrix;
    nativePen: GpPen;
  begin
  ...
  function TGPGraphicsPath.Warp(destPoints: PPointF; count: Integer; srcRect: TRectF;
            matrix: TGPMatrix = nil; warpMode: TWarpMode = WarpModePerspective;
            flatness: Single = FlatnessDefault): TStatus;
  var nativeMatrix: GpMatrix;
  begin
  ...
  function TGPGraphicsPath.GetPathPoints(points: PPointF; count: Integer): TStatus;
  begin
    result := SetStatus(GdipGetPathPoints(nativePath, points, count));
  end;

  function TGPGraphicsPath.GetPathPoints(points: PPoint; count: Integer): TStatus;
  begin
  ...
  function TGPGraphicsPath.IsVisible(point: TPointF; g: TGPGraphics = nil): BOOL;
  begin
  ...
  function TGPGraphicsPath.IsVisible(point: TPoint; g : TGPGraphics = nil): BOOL;
  begin
    result := IsVisible(point.X, point.Y, g);
  end;
  ...
  function TGPGraphicsPath.IsOutlineVisible(point: TPointF; pen: TGPPen; g: TGPGraphics = nil): BOOL;
  begin
  ...
  function TGPGraphicsPath.IsOutlineVisible(point: TPoint; pen: TGPPen; g: TGPGraphics = nil): BOOL;
  begin
    result := IsOutlineVisible(point.X, point.Y, pen, g);
  end;
...
//--------------------------------------------------------------------------
  ...
  function TGPGraphicsPathIterator.Enumerate(points: PPointF; types: PBYTE;
    count: Integer): Integer;
  begin
    SetStatus(GdipPathIterEnumerate(nativeIterator, result, points, types, count));
  end;

  function TGPGraphicsPathIterator.CopyData(points: PPointF; types: PBYTE;
...
//--------------------------------------------------------------------------
  ...
  constructor TGPPathGradientBrush.Create(points: PPointF; count: Integer; wrapMode: TWrapMode = WrapModeClamp);
  ...
  constructor TGPPathGradientBrush.Create(points: PPoint; count: Integer; wrapMode: TWrapMode = WrapModeClamp);
  var brush: GpPathGradient;
  begin
    brush := nil;
  ...
  function TGPPathGradientBrush.GetCenterColor(out Color: Tcolor): TStatus;
  begin
    SetStatus(GdipGetPathGradientCenterColor(GpPathGradient(nativeBrush), Color));
    result := lastResult;
  end;

  function TGPPathGradientBrush.SetCenterColor(color: TColor): TStatus;
  begin
  ...
  function TGPPathGradientBrush.GetCenterPoint(out point: TPointF): TStatus;
  begin
    result := SetStatus(GdipGetPathGradientCenterPoint(GpPathGradient(nativeBrush), @point));
  end;

  function TGPPathGradientBrush.GetCenterPoint(out point: TPoint): TStatus;
  begin
    result := SetStatus(GdipGetPathGradientCenterPointI(GpPathGradient(nativeBrush), @point));
  end;

  function TGPPathGradientBrush.SetCenterPoint(point: TPointF): TStatus;
  begin
    result := SetStatus(GdipSetPathGradientCenterPoint(GpPathGradient(nativeBrush), @point));
  end;

  function TGPPathGradientBrush.SetCenterPoint(point: TPoint): TStatus;
  begin
    result := SetStatus(GdipSetPathGradientCenterPointI(GpPathGradient(nativeBrush), @point));
  end;

  function TGPPathGradientBrush.GetRectangle(out rect: TRectF): TStatus;
  begin
    result := SetStatus(GdipGetPathGradientRect(GpPathGradient(nativeBrush), @rect));
  end;

  function TGPPathGradientBrush.GetRectangle(out rect: TRect): TStatus;
  begin
